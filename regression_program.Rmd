...
\begin{frame}{Modelo Linear Normal Homocedástico}
    \begin{table}
    \centering
    \resizebox{10cm}{!}{
...
Dadas as estimativas dos efeitos do tratamento serem negativas isso favorece  os efeitos do placebo, ou seja o placebo tem melhores resultados que o tratamento proposto. \\
    $R^2$:  0,9432 \ e \ $R^2$ ajustado :  0,9426
...
$$\boldsymbol{\cal{V}}_ \text{i} \equiv \boldsymbol{Z_} \text{i} \boldsymbol{\cal{D}} \boldsymbol{Z}_\text{i}^{\boldsymbol{'}} + \boldsymbol{\sigma}^\text{2} \boldsymbol{I}_\text{4}$$
...
```{r convergencia}
traceplot(matrixFit,pars=c("beta","sigma_e","sigma_p"))
```
