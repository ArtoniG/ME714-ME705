...
Facilmente encontramos na literatura que uma das abordagens utilizadas para modelar este tipo de estudo com medidas repetidas são modelos conhecidos como modelos de efeitos aleatórios ou modelos mistos (MM), como podemos encontrar nos trabalhos de @gelman, @manly, @gamerman e @singer. A estrutura deste tipo de modelo considera não somente efeitos fixos e a componente dos erros, como também uma componente aleatória que incorpora a variabilidade do comportamento individual da i-ésima unidade experimental. Este tipo de estrutura tem diversas aplicações, tais como em modelos lineares hierárquicos e modelos de efeitos contextuais utilizados em estudos nas áreas da Educação e Sociologia respectivamente.  
Afim de obter os resultados do ajuste do modelo utilizaremos uma abordagem da Inferência Bayesiana, através do método Monte Carlo Markov Chain (MCMC) implementado no software Stan integrado ao software R por meio do pacote *rstan*. Segundo @sorensen o Stan fornece um conjunto de ferramentas escaláveis para ajustar modelos na maioria das aplicações padrões de MM.  
No que segue apresentaremos a metodologia utilizada para investigar os desempenhos de ambos tratamentos, ou seja a estrutura dos modelo mistos, o método MCMC, uma análise descritiva dos dados, o modelo proposto, assim como os resultados obtidos com o mesmo.
